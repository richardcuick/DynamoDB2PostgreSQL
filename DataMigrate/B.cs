...
namespace DataMigrate
{
    public static class B
    {
        ...
        public static void PrintValue(AttributeValue value, int n)
        {
            ...
            {
                Console.Write("{\"NS\":{0}}", string.Join(",", value.NS.ToArray()));
            }
            ...
            {
                Console.Write($"{{\"S\":\"{value.S}\"}}");
            }
            // String set attribute value.
            else if (value.SS.Count > 0)
            {
                Console.Write("{\"SS\":{0}}", string.Join(" ", value.SS.ToArray()));
            }
            // Otherwise, boolean value.
            else
            {
                Console.Write($"{{\"B\":{ value.BOOL.ToString().ToLower()}}}");
            }
        }
    }
}
